...
function Profile() {
    ...
    const handleMarkComplete = async (opId) => {
        ...
        const newHistoryIds = historyIds.includes(opId)
            ? historyIds
            : [...historyIds, opId];
        ...
        await supabase
            .from("profiles")
            .update({
                future_events: newFutureIds,
                event_history: newHistoryIds,
            })
            .eq("id", userId);
    };
    ...
    const handleUndoComplete = async (opId) => {
        ...
        const newFutureIds = futureIds.includes(opId)
            ? futureIds
            : [...futureIds, opId];
    ...
    const removeSkill = (skill) =>
        handleChange(
            "skill_set",
            (profile?.skill_set || []).filter((s) => s !== skill)
        );
    ...
    const removeAccessibility = (item) =>
        handleChange(
            "accessibility_needs",
            (profile?.accessibility_needs || []).filter((s) => s !== item)
        );
    ...
    const handleSave = async () => {
        ...
        if (error) {
            ...
            setSuccess("Changes saved! You're all set ✨");
            setEditing(false);
        }
    };
    ...
    const manageableSummary = useMemo(() => {
        ...
        historyEvents.forEach((op) => {
            ...
            if (lt.some((t) => t.includes("alone") || t.includes("solo")))
                solo += 1;
            if (
                lt.some((t) => t.includes("quiet") || t.includes("small group"))
            )
                quiet += 1;
        });
        const items = [];
                    ...
                    <Card className="journey-hero-card">
                        <div className="journey-hero-top">
                            <div className="journey-hero-copy">
                                <p className="journey-kicker">YOUR SPACE</p>
                                <h1 className="journey-title">
                                    Here's a quick glance at what you've been
                                    exploring
                                </h1>
                                <p className="journey-subtitle">
                                    See what you’ve tried, what’s coming up, and
                                    what’s been helping you move forward.
                                </p>
                            </div>
                        ...
                        {!editing ? (
                            <>
                                <div className="journey-meta-row">
                                    {profile?.pronouns && (
                                        <span className="journey-meta-pill">
                                            {profile.pronouns}
                                        </span>
                                    )}
                                ...
                                {profile?.accessibility_needs?.length > 0 && (
                                    ...
                                    >
                                        {profile.accessibility_needs.map(
                                            (s) => (
                                                <Bubble
                                                    key={s}
                                                    label={`♿ ${s}`}
                                ...
                                <div className="journey-encouragement-card">
                                    ...
                                    <div>
                                        <p className="journey-encouragement-title">
                                            Every step counts
                                        </p>
                                        <p className="journey-encouragement-text">
                                            {encouragement}
                                        </p>
                                    </div>
                                </div>
                                ...
                                <Row className="g-3">
                                    ...
                                    <Col md={12}>
                                        <Form.Group>
                                            ...
                                            />
                                            <Form.Text className="text-muted">
                                                Use your device location or type
                                                one in. We’ll only use this to
                        ...
                        {error && (
                            <p className="journey-message journey-error">
                                {error}
                            </p>
                        )}
                        {success && (
                            <p className="journey-message journey-success">
                                {success}
                            </p>
                        )}
                    </Card>
                    ...
                    <Row className="g-4 mt-1 mb-4">
                        <Col md={6} xl={3} className="d-flex">
                            <Card className="journey-summary-card">
                                ...
                                <p className="journey-summary-label">
                                    Opportunities you've tried
                                </p>
                                <h3 className="journey-summary-value">
                                    {totalCompleted}
                                </h3>
                                <p className="journey-summary-subtext">
                                    experiences you’ve explored
                                </p>
                            </Card>
                        </Col>
                        <Col md={6} xl={3} className="d-flex">
                    ...
                    <Row className="g-4 mb-4">
                        <Col lg={7}>
                            <Card className="journey-section-card h-100">
                                <div className="journey-section-header">
                                    <div>
                                        <p className="journey-section-kicker">
                                            YOUR OPPORTUNITIES
                                        </p>
                                ...
                                {allMyEvents.length === 0 ? (
                                    <div className="journey-empty-state">
                                        <p className="journey-empty-title">
                                            Nothing here yet
                                        </p>
                                        <p className="journey-empty-text">
                                            When you choose something to try,
                                            it’ll show up here.
                                        </p>
                                    </div>
                                ) : (
                                    <div className="journey-opportunity-list">
                                        {allMyEvents.map((op) => (
                                            <div
                                                ...
                                                style={{
                                                    filter: op._completed
                                                        ? "grayscale(1)"
                                                        : "none",
                                                    opacity: op._completed
                                                        ? 0.65
                                                        : 1,
                                                    transition:
                                            ...
                                            >
                                                <div className="journey-opportunity-main">
                                                    <p className="journey-opportunity-org">
                                                        {op.organization}
                                                    </p>
                                                    <h4 className="journey-opportunity-title">
                                                        {op.title}
                                                    </h4>
                                                    <div className="journey-opportunity-meta">
                                                        ...
                                                        {!op._completed && (
                                                            <span
                                                                className={`journey-effort-pill journey-effort-pill--${
                                                                    op.effortLevel ||
                                                                    1
                                                                }`}
                                                            >
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <span
                                                            className={`journey-check-box${
                                                                op._completed
                                                                    ? " journey-check-box--checked"
                                                                    : ""
                                                            }`}
                                                        >
                                                            {op._completed && (
                            ...
                            <Card className="journey-section-card h-100">
                                <div className="journey-section-header">
                                    <div>
                                        ...
                                        <h3 className="journey-section-title">
                                            What this has been helping you
                                            practice
                                        </h3>
                                    </div>
                                </div>
                                ...
                                {reflectionThemes.length === 0 ? (
                                    <div className="journey-empty-state">
                                        ...
                                        <p className="journey-empty-text">
                                            As you try out new opportunities,
                                            this section will reflect the kinds
                                            of steps you've been taking.
                                        </p>
                                    </div>
                                ) : (
                                ...
                                {manageableSummary.length > 0 && (
                                    <div className="journey-manageable-block">
                                        <p className="journey-manageable-title">
                                        What’s been feeling more comfortable
                                        </p>
                                        <div className="journey-manageable-tags">
                                            {manageableSummary.map((item) => (
                                                <span
                                                    key={item}
                                                    className="journey-manageable-pill"
                                                >
                                                    {item}
                                                </span>
                                            ))}
                ...
                <Modal.Body className="opportunity-modal-body p-0">
                    {selectedOpportunity && (
                        <OpportunityCard
                            {...selectedOpportunity}
                            isCommitted={
                                futureIds.includes(selectedOpportunity.id) ||
                                historyIds.includes(selectedOpportunity.id)
                            }
                            isCompleted={historyIds.includes(
